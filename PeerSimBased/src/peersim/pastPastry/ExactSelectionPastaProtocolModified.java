 ...
 */
public class ExactSelectionPastaProtocolModified extends PastaProtocol implements Cloneable{
    ...
    public ExactSelectionPastaProtocolModified(String prefix){
        super(prefix);
    }
    public Object clone(){
        ExactSelectionPastaProtocolModified obj = null;
       
        obj = (ExactSelectionPastaProtocolModified)super.clone();
    ...
    protected void computeCharges() {
       nodesCharge.clear();
        Node node;
        ExactSelectionPastaProtocolModified prot;
        for (int i = 1; i <=selectionRange; ++i) {
            node = Network.get(Util.modulo(me.getIndex()+i,Network.size()));
             prot = (ExactSelectionPastaProtocolModified) node.getProtocol(pid);
            nodesCharge.add(new NodeCharge(node,prot.storage.size() ));
        ...
        nodesCharge.add(new NodeCharge(me, storage.size()));
    }
}
