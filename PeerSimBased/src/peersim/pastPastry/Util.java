 ...
 * @version 1.0
 * veronique Simon : added modulo function to retreive neighbours nodes
 */
public class Util {
    ...
    public static final int charToIndex(char c) {
     ...
     public static final int delay(int size,double bandwidth){
    	 return (int)((new Integer(size).doubleValue())/bandwidth);
     }
     
     public static final Node[] copyTab(Node []src){
    	 Node []dest=new Node[src.length];
     ...
     public static Node nodeIdtoNode(BigInteger searchNodeId, int layerid) {
         ...
         while (inf <= sup) {
             m = (inf + sup) / 2;
             BigInteger mId = ((MSPastryProtocolImpl) Network.get(m).getProtocol(layerid)).nodeId;
         ...
         for (int i= Network.size()-1; i >= 0 ; i--) {
               mId = ((MSPastryProtocolImpl) Network.get(i).getProtocol(layerid)).nodeId;
              if  (mId.equals(searchNodeId)){
 	...
 	public static int getRandomIndex(Random r,int allowedRange){
		int index;
		int middle=(allowedRange)/2;
		
		index=r.nextInt(allowedRange);
		index++;
